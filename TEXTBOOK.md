...
MCP's `2026-07-28` revision makes the transport difference smaller than it used to be. The modern protocol has **no initialize handshake and no protocol-level session**. Every request carries its version, client identity, and capabilities in `_meta`; an optional `server/discover` call can fetch server capabilities up front. Over HTTP, `MCP-Protocol-Version`, `Mcp-Method`, and `Mcp-Name` make the operation visible to a gateway without requiring it to parse JSON. There is no `Mcp-Session-Id`, so the load balancer can send each request to any healthy replica without sticky routing or a shared session table.

Stateless protocol does not mean stateless application. If a server starts a job or workflow, it should mint an explicit handle and make that handle a later tool argument. That is better for a model-facing system than invisible transport state: the model can see the handle, preserve it in its transcript, and supply it deliberately. The SDK's `stateless_http=True` option remains only to control how pre-2026 legacy clients are served; modern traffic is already self-contained.
