...
"Almost nothing" hides one decision, and it is a decision you have met before. Going over the network gives the connection a **session**: the server mints an id on the first request, returns it in an `Mcp-Session-Id` header, and the client repeats it from then on, so the server can find the state that belongs to this particular client. A subprocess never needed that, because the process *was* the connection; when it dies, the state dies with it. Over HTTP the state has to be findable, and an id is how you find it.

The trouble arrives the day one server process is not enough. State that lives inside a particular process makes that process the only one that can continue the conversation, so the load balancer in front of your three replicas has to keep sending each client back to the replica it started with. If that sounds familiar, it should: it is the sticky-session problem that has been making web services awkward to scale since long before anyone thought of putting a model behind one. Chapter 1 met the same problem from the other side, where the model API's answer was to hold no state at all and make you resend the transcript every turn. MCP offers you the same escape hatch, spelled `stateless_http=True`. Sessions stop being issued, each request is served by a transport built for it and thrown away after, and any replica can answer anything.

What you pay is worth being precise about, because it is not "features" in general. Notifications a server sends while handling a call, progress updates on a long computation, still work, since they ride the response stream of the request that is already open. What stops working is anything that needs the connection to outlive the request: streams a client can reconnect to and replay, and requests the *server* makes of the *client*, such as asking the host to sample from its model mid-tool. In stateless mode the server can still speak, but the client's answer has nowhere to arrive, so the SDK refuses the attempt outright rather than hanging. The useful question is therefore not which mode is better but a structural one about your server: does it ever need to talk back outside of answering a call? A server that computes and returns should be stateless and scale like any other web service. A server that interviews the user or streams a job you might reconnect to needs the session, and the stickiness is the price of it.
