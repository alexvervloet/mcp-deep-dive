...
`session.list_tools()` (the `tools/list` request — discover what's available) and
`session.call_tool(name, args)` (the `tools/call` request — run one). Everything
...
can ship and improve a great "summarize my notes" prompt — right field names, right
...
Note that `save_note` has a **side effect** (writes a file) — flag it for approval
